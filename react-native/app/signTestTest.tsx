...
import useEffectOnce from './vendor/useEffectOnce'
...
export default function SignTest() {
  ...
  useEffectOnce(() => {
    if (account.privateKey) {
      setWallet(new Wallet(account.privateKey))
    } else {
      setError('no private key present in account')
    }
  })

  const signMessage = (msgParams: any) => {
    if (!wallet) {
      setError('no wallet present')
      return
    }
    wallet
      .signMessage(msgParams)
      .then((result: any) => {
        console.log('sign msg result', result)
      })
      .catch((e) => {
  ...
  async function test1() {
    ...
    const signedDelegation = signMessage(JSON.stringify(msgParams))
      ...
      batch: [
        {
          ...
          transaction: {
            to: msgParams.domain.verifyingContract,
  ...
  async function test2() {
    ...
    const signedDelegation = signMessage(JSON.stringify(eip712Data))
      ...
      batch: [
        {
          ...
          transaction: {
            to: eip712Data.domain.verifyingContract,
            gasLimit: '10000000000000000',
            // data: tx,
          },
        },
  ...
  //     const params = [account.address, msgParams]
  ...
  return (
    <Main>
      ...
      <Section>{error ? <Text>{error}</Text> : null}</Section>
