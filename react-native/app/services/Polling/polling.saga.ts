...
export function* pollingWorker(payload: IPollingPayload) {
  console.debug('[pollingWorker]: ', payload)
  const { startAction, stopAction, saga, params } = payload

  let retriesCount = 0

  console.debug(
    `[${startAction.type}]: Initialising polling for every ${params.interval}ms `,
  )
  while (true) {
    try {
      ...
      const shouldRetry =
        params.retryOnFailure && params.retries
          ? retriesCount < params.retries
          : false // Determine if the polling should retry on error
