...
const fromTokenBase = (
  value: TokenValue,
  decimal: number = DEFAULT_ASSET_DECIMAL,
...
const totalTxFeeToWei = (
  gasPriceEther: string | Bigish,
  gasLimit: string | Bigish,
): Wei => bigify(gasPriceEther).multipliedBy(gasLimit)

const gasStringsToMaxGasBN = (
  gasPriceGwei: string,
  gasLimit: string,
): BigNumber => {
...
// @todo Rename
const gasStringsToMaxGasNumber = (
  gasPriceGwei: string,
  gasLimit: string,
): BigNumber => {
  return bigify(
