...
export const inputGasLimitToHex = (
  gasLimit: string | BigifySupported,
): ITxGasLimit => addHexPrefix(bigify(gasLimit).toString(16)) as ITxGasLimit

export const inputValueToHex = (valueEther: string): ITxValue =>
  addHexPrefix(
    toTokenBase(valueEther, DEFAULT_ASSET_DECIMAL).toString(16),
  ) as ITxValue
...
export const hexNonceToViewable = (nonceHex: string): string =>
  hexToString(nonceHex)

export const hexToString = (hexValue: string): string =>
  bigify(hexValue).toString()

export const hexWeiToString = (hexWeiValue: string): string =>
  Wei(hexWeiValue).toString()
...
export const bigNumGasLimitToViewable = (
  gasLimitBigNum: Bigish | string,
): string => bigify(gasLimitBigNum).toString()

export const bigNumValueToViewableEther = (
  valueWeiBigNum: BigifySupported | string,
): string => formatEther(EthersBigNumber.from(valueWeiBigNum.toString()))
